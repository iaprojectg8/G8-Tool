...
def widget_init():
    ...
    variables_chosen = st.multiselect("Choose the variable you want to extract", READABLE_VARIABLE, default=READABLE_VARIABLE)
    model = st.selectbox(label="Chose the model to use", options=MODEL_NAMES_CMIP6)
    ...
    return variables_chosen, model, ssp, experiment
...
def make_whole_request(bounds, years):

    variables_chosen, model, ssp, experiment = widget_init()
    ...
    for variable in variables_chosen:
        for year in years:
            make_year_request(variable, model, ssp, experiment, bounds)

def make_year_request(variable_chosen, model, ssp, experiment, bounds, year):
    """
    ...
    base_url = (f"https://ds.nccs.nasa.gov/thredds/ncss/grid/AMES/NEX/GDDP-CMIP6/{model}/{ssp}/{experiment}/{variable_chosen}/"
                    f"{variable_chosen}_day_{model}_{ssp}_{experiment}_gr_{year}.nc")
    ...
    coordinates_part=(f"?var=pr&north={params["north"]}&west={params["west"]}&east={params["east"]}&south={params["south"]}&"
                      "horizStride=1&time_start=2014-01-01T12:00:00Z&time_end=2014-12-31T11:00:00Z&&&accept=netcdf3&addLatLon=true")
    
    url = f"{base_url}{coordinates_part}"
    # print("\n",url,"\n")
    response = requests.get(url)
    try:
        with open(f"{NC_FILE_DIR}/{year}.nc", "wb") as f:
